...
class MoMKE(nn.Module):
    ...
    def __init__(self, args, adim, tdim, vdim, D_e, n_classes, depth=4, num_heads=4, mlp_ratio=1, drop_rate=0, attn_drop_rate=0, no_cuda=False):
        ...
        self.dropout_a = nn.Dropout(args.dropout)
        self.dropout_t = nn.Dropout(args.dropout)
        self.dropout_v = nn.Dropout(args.dropout)
    ...
    def forward(self, inputfeats, input_features_mask=None, umask=None, first_stage=False):
        ...
        weight_save = []
        # sequence modeling
        audio, text, video = inputfeats[0][:, :, :self.adim], inputfeats[0][:, :, self.adim:self.adim + self.tdim], \
        inputfeats[0][:, :, self.adim + self.tdim:]
        ...
        # --> [batch, seqlen, 3]
        input_mask = torch.clone(input_features_mask[0].permute(1, 0, 2))
